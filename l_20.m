...
for k=1:1
    ...
    k1 = randi(n-L+1);
    k2 = randi(m-L+1);
    crop = img(k1+(0:L-1),k2+(0:L-1), :);
    imwrite(crop,"kk.png");
    crop_dat = imageDatastore(["kk.png"]);
    imds_cat = imageDatastore(cat(1, x.Files, crop_dat.Files)); 
    imds_cat.Labels = cat(1, x.Labels, crop_dat.Labels);
    x = imds_cat;
    fclose("all");
    delete "kk.png";
    ...
    imds_cat1 = imageDatastore(cat(1, y.Files, crop_dat1.Files)); 
    imds_cat1.Labels = cat(1, y.Labels, crop_dat1.Labels);
    y = imds_cat1;
    fclose("all");
    delete "kk1.png";
end
...
%imdsCombined = randomPatchExtractionDatastore(x,y,[48 48], ....
%'PatchesPerImage',64);
...
tempLayers1 = [
    transposedConv2dLayer(2,64,'Stride',2,"Name","decon")
    convolution2dLayer([1 1],3,"Name","con_end")
    maeRegressionLayer("end")];
lgraph = addLayers(lgraph,tempLayers1);
...
disp("ok");
[ net, info ] = trainNetwork(imdsCombined,lgraph,options);
